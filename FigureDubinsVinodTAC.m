...
legend_cell = {'Target tube','Chance const.', 'Nominal trajectory'};
legend([h_target_tube, h_poly, h_nominal_traj], legend_cell, 'Location', legend_loc, 'interpreter','latex');
if savefigures
...
if ~isEmptySet(polyt)
    %for direction_index_to_plot = 1:2:no_of_direction_vectors_ccc
        init_state = extra_info_cheby.vertices_underapprox_polytope(:,direction_index_to_plot);
        input_vec = extra_info_cheby.opt_input_vec_at_vertices(:,direction_index_to_plot);
        opt_reach_avoid = extra_info_cheby.opt_reach_prob_i(direction_index_to_plot);
        ...
        [mean_X_zi, ~] = SReachFwd('concat-stoch', sys_no_input, init_state,...
            time_horizon);
        optimal_mean_X = mean_X_zi + H * input_vec;
